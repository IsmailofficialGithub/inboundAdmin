...
const getRevenueReport = async (req, res) => {
  try {
    ...
    // Base query for invoices - need to join through user_subscriptions to get package info
    let invoiceQuery = supabaseAdmin
      .from('invoices')
      .select('*, user_subscriptions(subscription_packages(package_name, package_code))')
      .eq('status', 'paid')
    ...
    ;(invoices || []).forEach((invoice) => {
      ...
      // Revenue by package - handle nested relationship
      const subscription = invoice.user_subscriptions
      if (subscription && subscription.subscription_packages) {
        const packageName = subscription.subscription_packages.package_name || 'Unknown'
        const packageCode = subscription.subscription_packages.package_code || 'unknown'
  ...
  try {
    ...
    if (userIds.length > 0) {
      const { data: subscriptions } = await supabaseAdmin
        ...
        .select('user_id, subscription_packages(package_name, package_code)')
        .in('user_id', userIds)
        .eq('status', 'active')

      ;(subscriptions || []).forEach((sub) => {
        if (!userSubscriptionsMap[sub.user_id]) {
          userSubscriptionsMap[sub.user_id] = sub
        }
      })
    }
...
const exportSubscriptions = async (req, res) => {
  try {
    ...
    let query = supabaseAdmin
      .from('user_subscriptions')
      .select('*, subscription_packages(package_name, package_code)')
    ...
    const subscriptionsWithEmail = await Promise.all(
      (subscriptions || []).map(async (sub) => {
        ...
        return {
          ...sub,
          email,
          package_name: sub.subscription_packages?.package_name || 'Unknown',
          package_code: sub.subscription_packages?.package_code || 'unknown',
        }
      })
    )
